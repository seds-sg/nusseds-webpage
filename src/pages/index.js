...
const IndexPage = () => {
	...
	useEffect(() => {
		const intervalHandle = setInterval(() => setCurrentIndex((prevIndex) => (prevIndex + 1) % 2), 10000)
		return () => {
	...
	return (
		<Layout>
			{[ 0, 1 ].map((projectIndex) => (
				<BgImgContainer key={projectIndex} selected={projectIndex === currentIndex}>
						...
						<StyledNavBarPadding>
							...
							<div>
								...
								<ProjectDetailsButton
									onClick={() => (window.location = projectIndex === 0 ? "/new-oppy" : "darpa-subt")}
